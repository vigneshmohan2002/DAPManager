...
def find_and_resolve_duplicates(db: DatabaseManager):
    ...
    if paths_to_delete:
        with open("files_to_delete.txt", "w", encoding="utf-8") as f:
            for path in paths_to_delete:
                f.write(f"{path}\n")
        logger.info(f"Generated 'files_to_delete.txt' with {len(paths_to_delete)} paths.")
    ...
    if unresolved_patterns:
        ...
        logger.info(f"Generated 'unresolved_duplicates.csv' with {len(unresolved_patterns)} groups.")
    ...
    logger.info(f"Unresolved groups (Ties): {len(unresolved_patterns) - sanity_failures}")
