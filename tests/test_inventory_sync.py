...
from src.inventory_sync import INVENTORY_REPORT_STATE_KEY, main_run_inventory_report
...
def test_master_rejection_raises(db):
    ...
    with patch("src.inventory_sync.requests.Session.post",
        ...
        with pytest.raises(RuntimeError, match="nope"):
            main_run_inventory_report(db, config)
    assert db.get_sync_state(INVENTORY_REPORT_STATE_KEY) is None
...
def test_session_retries_only_post_for_transient_server_errors():
    from src.inventory_sync import _session
...
def test_empty_report_replaces_previous_snapshot_and_reports_progress(db):
    db.replace_device_inventory(
        "master-1", [{"mbid": "stale", "local_path": "/old"}]
    )
    events = []

    summary = main_run_inventory_report(
        db,
        {"device_id": "master-1", "device_role": "master"},
    ...
    assert db.get_device_inventory("master-1") == []
    assert db.get_sync_state(INVENTORY_REPORT_STATE_KEY) is not None
    assert events == [{"message": "Recording own inventory (0 items)"}]
